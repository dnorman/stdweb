...
use webapi::error;
use futures2::{Future, Poll, Async};
use futures2::unsync::oneshot::Receiver;
...
///
/// The preferred way to create a `PromiseFuture2` is to use [`value.try_into()`](unstable/trait.TryInto.html) on a JavaScript [`Value`](enum.Value.html).
///
/// # Examples
///
/// Convert a JavaScript `Promise` into a `PromiseFuture2`:
///
/// ```rust
/// let future: PromiseFuture2<String> = js!( return Promise.resolve("foo"); ).try_into().unwrap();
/// ```
pub struct PromiseFuture2< Value, Error = error::Error > {
...
impl< A, B > std::fmt::Debug for PromiseFuture2< A, B > {
    fn fmt( &self, formatter: &mut std::fmt::Formatter ) -> std::fmt::Result {
        write!( formatter, "PromiseFuture2" )
    }
}

impl< A, B > Future for PromiseFuture2< A, B > {
    type Item = A;
    type Error = B;
...
impl< A, B > TryFrom< Value > for PromiseFuture2< A, B >
